...
from modules.utils.tasks import MigalkiThemeTask
from Models.Tradingplatform import Tradingplatform


class MigalkiParser:
    _DOMAIN = r'http://migalki.pw'
    ...
    def collect_process(self, task):
        ...
        for page in self._client.search():
            ...
            for theme in themes_block:
                ...
                link = urljoin(self._DOMAIN, row.find('a', href=True)['href'])
    ...
    def save_theme_info(self, bs):
        ...
        for message in messages:
            if message.select_one('.message-name').text == seller_name:
                ...
                if medias:
                    ...
                    for media in medias:
                        path = 'Media/{}/{}'.format(title, media.get('src').split('/')[-1])
